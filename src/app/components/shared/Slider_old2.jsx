...
// Импортируем компоненты Swiper для React
import { Swiper, SwiperSlide } from "swiper/react";
// Импортируем модуль навигации (стрелки)
...
// Массив с моковыми данными (замените ссылки на свои изображения)
const testimonials = [
  {
    id: 1,
    name: "Bang Upin",
...
export default function Slider() {
  return (
    <section className="relative max-w-360 mx-auto bg-[#FAFAFA] py-20 px-4">
      {/* Главный контейнер. Добавлен padding по бокам, чтобы стрелкам было место */}
      <div className="mx-auto relative px-10 md:px-20">
        <Swiper
          modules={[Navigation]}
          navigation={{
        ...
        >
          {testimonials.map((item) => (
            <SwiperSlide key={item.id} className="pt-10 pb-10">
              {/* === КАРТОЧКА === */}
              <div className="relative h-[520px] mx-auto group">
                ...
                <div className="relative w-full h-full rounded-[32px] overflow-hidden z-10 shadow-sm">
                  ...
                  <div className="absolute bottom-5 left-5 right-5 bg-white rounded-[24px] px-6 pb-8 pt-12 text-center shadow-lg">
                    ...
                    <p className="text-[#1E1E1E] text-[14px] leading-relaxed mb-6 font-medium">
                      {item.quote}
                    </p>
        ...
        <button className="swiper-button-prev-custom absolute left-0 md:left-4 top-1/2 -translate-y-1/2 w-14 h-14 bg-white rounded-full flex items-center justify-center shadow-[0_8px_30px_rgb(0,0,0,0.1)] hover:bg-gray-50 transition-colors z-20 cursor-pointer">
          <svg
        ...
        <button className="swiper-button-next-custom absolute right-0 md:right-4 top-1/2 -translate-y-1/2 w-14 h-14 bg-white rounded-full flex items-center justify-center shadow-[0_8px_30px_rgb(0,0,0,0.1)] hover:bg-gray-50 transition-colors z-20 cursor-pointer">
          <svg
            className="w-6 h-6 text-black"
            fill="none"
            stroke="currentColor"
